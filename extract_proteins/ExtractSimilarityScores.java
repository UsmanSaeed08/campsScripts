...
import extract_proteins.ReadHitsThread;
import datastructures.TMS;
import de.gsf.mips.simap.lib.database.DbSequence;

public class ExtractSimilarityScores {
	...
	public void run(String logfile) {
		...
		try {
			...
			for (int sequenceid = selected_sequences.nextSetBit(0); sequenceid >=0; sequenceid = selected_sequences.nextSetBit(sequenceid+1)) {
				...
				while (ReadHitsThread.getThreadCount() >= N_SIMAP_THREADS) { // a beautiful way to control the number of threads being generated
					try {
						Thread.sleep(100);
				...
				new ReadHitsThread(sequenceid, selected_sequences, mapping, searchSpaceSize, sequencelengths, CAMPS_CONNECTION, MAX_EVALUE, MIN_LENGTH_RATIO, campsid2tms);			
			}
			
			//wait until all threads are finished
			while(ReadHitsThread.getThreadCount() > 0) {
				try {
					Thread.sleep(100);
				} catch(InterruptedException e) {
					e.printStackTrace();
			...
			System.err.println("Exception in ExtractSimilarityScores.run(): " +e.getMessage());
			e.printStackTrace();
		}
		finally {
